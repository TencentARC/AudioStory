...
class Flux_T5_cotrain(nn.Module):
    ...
    def __init__(self, config, text_encoder_dir=None, initialize_reference_model=False,):
        ...
        self.text_encoder_name = config.get("text_encoder_name", "ckpt/flan-t5-large")
    ...
    def encode_text_classifier_free(self, prompt: List[str], T5_tokens_seedx, num_samples_per_prompt=1):
        device = self.transformer.device
        dtype = self.transformer.dtype

        T5_tokens_seedx = T5_tokens_seedx.to(device, dtype=dtype)
        ...
        padding_length = 64-attention_mask.size()[-1]
        attention_mask_seedx = F.pad(attention_mask, (0, padding_length), "constant", 0)==1

        T5_tokens_seedx = T5_tokens_seedx[attention_mask_seedx].unsqueeze(dim=0)
        prompt_embeds_seedx = T5_tokens_seedx.repeat_interleave(num_samples_per_prompt, 0)

        mse = F.mse_loss(prompt_embeds.detach(), T5_tokens_seedx.detach())
        print('mse loss:', mse)
        ...
        max_length = prompt_embeds_seedx.shape[1]
        ...
        prompt_embeds_seedx = torch.cat([negative_prompt_embeds, prompt_embeds_seedx])
        prompt_mask = torch.cat([uncond_attention_mask, attention_mask])
        boolean_prompt_mask = (prompt_mask == 1).to(device)

        return prompt_embeds_seedx, boolean_prompt_mask, prompt_embeds
    ...
    def encode_text(self, prompt):
        ...
        return encoder_hidden_states, boolean_encoder_mask #[16,1024] [2,16]
    ...
    def inference_flow(
        ...
        if classifier_free_guidance:
            ...
            encoder_hidden_states, boolean_encoder_mask, gt_prompt_embeds = (
                self.encode_text_classifier_free(
                    prompt, T5_tokens_seedx, num_samples_per_prompt=num_samples_per_prompt
                )
            )
        ...
        for i, t in enumerate(timesteps):
            ...
            latents_input = latents_input.to(self.transformer.device, self.transformer.dtype)
    ...
    def encode_text_classifier_free_padding(self, prompt: List[str], T5_tokens_seedx, padding_length, num_samples_per_prompt=1):
        device = self.transformer.device
        dtype = self.transformer.dtype
        ...
        mse = F.mse_loss(prompt_embeds.detach(), T5_tokens_seedx.deatch())
        print('mse loss:', mse)
    ...
    def inference_flow_padding(
        ...
        if classifier_free_guidance:
            ...
            encoder_hidden_states, boolean_encoder_mask, gt_prompt_embeds = (
                self.encode_text_classifier_free_padding(
                    prompt, T5_tokens_seedx, padding_length, num_samples_per_prompt=num_samples_per_prompt
                )
            )
            duration_hidden_states = duration_hidden_states.repeat(bsz, 1, 1)
        ...
        return latents, gt_prompt_embeds




    ####################################### full 64 tokens no mask ##############################

    def encode_text_classifier_free_full_tokens(self, T5_tokens_seedx, num_samples_per_prompt=1):
        device = self.transformer.device
        dtype = self.transformer.dtype
        ...
        # T5_tokens_seedx = T5_tokens_seedx
        prompt_embeds_seedx = T5_tokens_seedx.repeat_interleave(num_samples_per_prompt, 0)
        ...
        return prompt_embeds_seedx



    @torch.no_grad()
    def inference_flow_full_tokens(
        self,
        prompt,
        T5_tokens_seedx,
        num_inference_steps=50,
        ...
        if classifier_free_guidance:
            ...
            encoder_hidden_states = (
                self.encode_text_classifier_free_full_tokens(
                    T5_tokens_seedx, num_samples_per_prompt=num_samples_per_prompt
                )
            )
            duration_hidden_states = duration_hidden_states.repeat(bsz, 1, 1)
        ...
        pooled = torch.nanmean(encoder_hidden_states, dim=1)
        pooled_projection = self.fc(pooled)
 ...
 ########################### ########################### ###########################
    ...
    def forward(self, latents, t5_tokens, duration=torch.tensor([10]), sft=True):

        device = latents.device
        audio_seq_length = self.audio_seq_len
        bsz = latents.shape[0]

        encoder_hidden_states = t5_tokens # torch.Size([2, 26, 1024]) torch.Size([2, 26])
        duration_hidden_states = self.encode_duration(duration)
        ...
        if sft:
            ...
            noise = torch.randn_like(latents) # latents：[bz, 645, 64]
    ...
    def forward_fake(self, latents, t5_tokens, duration=torch.tensor([10]), sft=True):
