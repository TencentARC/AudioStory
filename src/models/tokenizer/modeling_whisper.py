...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        # modules = [nn.Linear(1280, 4096), nn.GELU(), nn.Linear(4096, 4096)]
        # self.aud_projector = nn.Sequential(*modules)
    ...
    def forward(
        ...
        # hidden_states_projected = self.aud_projector(hidden_states)
        if output_hidden_states:
            encoder_states = encoder_states + (hidden_states,)
...
class WhisperModel(WhisperPreTrainedModel):
    ...
    # @classmethod
    # def from_pretrained(cls, config,  pretrained_model_path=None, **kwargs):
    #     model = cls(config = WhisperConfig, **kwargs)
...
class Whisper_Resampler_llava(nn.Module):
    def __init__(self, token_num, ori_embed_dim, tgt_embed_dim):
        super(Whisper_Resampler_llava, self).__init__()
        self.token_num = token_num
        
        self.avg_pool = nn.AdaptiveAvgPool2d([self.token_num, ori_embed_dim])  # 5x1 window

        self.ori_embed_dim = ori_embed_dim
        self.tgt_embed_dim = tgt_embed_dim
        
        modules = [
            nn.Linear(ori_embed_dim, tgt_embed_dim),
            nn.GELU(),
            nn.Linear(tgt_embed_dim, tgt_embed_dim)
        ]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [batch_size, 1500, 1280]
        
        pooled = self.avg_pool(x)  # [batch_size, 1280, 300, 1]
        output = self.aud_projector(pooled)  # [batch_size, 300, 5120]
...
class Whisper_Resampler_llava_old(nn.Module):
    def __init__(self, window_size, ori_embed_dim, tgt_embed_dim):
        super(Whisper_Resampler_llava, self).__init__()
        self.window_size = window_size
        self.token_num = 1500/window_size
        
        self.avg_pool = nn.AvgPool2d(kernel_size=(window_size, 1), stride=(window_size, 1))  # 5x1 window
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # if self.token_num == 1500:
        #     pooled = x
        # else:
        # [batch_size, channels, height, width]
        x = x.permute(0, 2, 1).unsqueeze(3)  # [batch_size, 1280, 1500, 1]
        
        pooled = self.avg_pool(x)  # [batch_size, 1280, 300, 1]
        
        pooled = pooled.squeeze(3).permute(0, 2, 1)  # [batch_size, 300, 1280]
